...
var App = function (_React$Component) {
    ...
    _createClass(App, [{
        ...
        value: function addTodo(val) {
            var todo = {
                text: val,
                id: _uuid2.default.v4() //random id
            };
            var data = [].concat(_toConsumableArray(this.state.data), [todo]); //new array with pushed -todo-
        ...
        value: function removeTodo(id) {
            var remainder = this.state.data.filter(function (todo) {
                return todo.id !== id;
            }); //new array without one id to remove 
            this.setState({ data: remainder });
        }
    }, {
        key: 'render',
        value: function render() {
            //div style without quotation marks
            return _react2.default.createElement(
                'div',
                { className: _App2.default.TodoApp },
                _react2.default.createElement(_Title2.default, { title: 'tytuł', length: this.state.data.length })
            );
        }
    }]);
